...
def gtzan_resampler(genre, path):
    src, _ = load_audio(
        path=os.path.join(DATASET,'gtzan','wav', genre, path),
...
def GTZAN_processor(gtzan_path):
    ...
    torch.save(gtzan_final.to_dict('index'), os.path.join(gtzan_path, 'annotation.pt')) # 183M
    torch.save({i:j for i,j in Counter(gtzan_final['tag']).most_common()}, os.path.join(DATASET, "supervision", "gtzan_tag_stats.pt"))
    torch.save({i:"genre" for i in set(gtzan_final['tag'])}, os.path.join(DATASET, "supervision", "gtzan_tag_info.pt"))
